...
namespace secraorg
{
    public partial class PESQUISARDECLARACAOSIMPLES : Form
    {
        private MySqlConnection bdConn;
        private DataTable table;

        public PESQUISARDECLARACAOSIMPLES(MySqlConnection conex)
        {
        ...
        private void dataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex >= 0 && dataGridView1.SelectedRows.Count > 0)
            {
                if (dataGridView1[0, e.RowIndex].Value.ToString() != "")
                {
                    ...
                    DECLARACAOSIMPLES declaracaosimples = new DECLARACAOSIMPLES(bdConn, dataGridView1[0, e.RowIndex].Value.ToString(), this);
                    table.Clear();
        ...
        private void PESQUISARDECLARACAOSIMPLES_FormClosing(object sender, FormClosingEventArgs e)
        {
            DECLARACAOSIMPLES voltdeclaracaosimples = new DECLARACAOSIMPLES(bdConn);
            table.Clear();
            voltdeclaracaosimples.Show();
            this.Dispose();
        }
